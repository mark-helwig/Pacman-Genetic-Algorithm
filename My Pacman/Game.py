...
class Game:
    def __init__(self):
        ...
        self.pacman = Pacman(self.window,self.filename, spritesheet, self.nodeMap)
        ...
        '''
        self.pinky = AStarGhost(self.window,self.filename, self.pacman, spritesheet, PINKY, self.nodeMap)
        self.inky = UCSGhost(self.window,self.filename, self.pacman, spritesheet, INKY, self.nodeMap)
    ...
    def run(self):
        ...
        while self.running:
            pygame.display.update()
            for event in pygame.event.get():
                self.pacman.keyboardMove(event)
